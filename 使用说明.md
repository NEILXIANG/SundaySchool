...
## 🚀 使用步骤
1. **准备照片**
   - 将每位学生的单人照命名为 `姓名_序号.jpg`（如 `ZhangSan_1.jpg`），全部放入 `input/student_photos/`
   - 将所有课堂多人合照放入 `input/class_photos/`
...
2. **运行程序**
   ...
   python src/run.py
   ```
   （默认 `input_dir="input"`，`output_dir="output"`，`log_dir="logs"`）
...
```bash
# 自定义输入目录
python src/run.py --input-dir my_input
...
## 📊 示例数据
项目已提供示例文件，可直接测试：
- `input/student_photos/` 下有 ZhangSan、LiSi、WangWu 的示例单人照
- `input/class_photos/` 下有示例课堂合照
...
**Q: 程序找不到学生照片？**  
A: 检查 `input/student_photos/` 是否存在，且文件名前缀为学生姓名（如 `ZhangSan_1.jpg`）。
