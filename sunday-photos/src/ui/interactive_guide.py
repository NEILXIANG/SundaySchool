...
按回车键开始...
    ...
    def check_student_photos(self):
        ...
        print("💡 参考照是可选的：如果暂时没有参考照也能运行（课堂照片会全部归入 unknown）")
        return True
...
🎯 目标：将照片文件名改为 姓名.扩展名 或 姓名_序号.扩展名（序号可选）
...
📝 文件命名：
    ...
    • 示例：Alice.jpg、Alice_2.jpg、Bob.jpg
    • 建议：如使用序号，从 1 开始，避免前导零（如 _01）
    • 注意：名字和序号之间用下划线连接（如 Alice_2.jpg）
