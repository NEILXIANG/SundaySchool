...
- 或（控制台版）首次推荐双击 `启动工具.command`（备选：`启动工具.sh`）
...
参考排障：优先查看 `HealthCheck_Runtime.md`，按“发生了什么 → 下一步 → 日志位置”顺序操作。
...
参考照质量要点（不要让老师调阈值）：
- 清晰正脸、单人、无遮挡、光线均匀；避免侧脸/合照/强背光。
- 每人 1-5 张即可；识别不准优先补高质量参考照，而不是改参数。
...
### Q6: 我看到 unknown_photos 很多，是不是识别坏了？
不一定。

`unknown_photos/` 里可能混有三类情况：
- 未识别（unknown）
- 无人脸（no-face，例如投影/风景/背影）
- 处理出错（error，例如文件损坏）

怎么判断：
1) 打开 `output/` 里最新的 `*_整理报告.txt`，看三类统计的拆分
2) 对“应该识别出来的学生”，优先补 2–3 张更清晰的参考照，然后再跑一遍

### Q7: 第一次运行提示 “Matplotlib is building the font cache”
这是正常现象：第一次运行会生成一次字体缓存。

老师版 `.app` 已做优化：字体缓存会写到 `logs/mplconfig/`；第一次完成后，后续启动会明显更快。
...
建议：
1) 先等 1–3 分钟，看 `logs/` 是否持续增长（通常仍在处理）
2) 仍不动就把 `logs/` 发给技术同工（不要自己反复双击多次）

（更短的“照做版”请看 `doc/TeacherQuickStart.md`）
