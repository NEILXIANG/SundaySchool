...
- 程序会在**可执行文件同目录**创建工作目录结构：
   ...
   - `output/`：整理结果
   - `logs/`：日志（遇到问题时用）
   - 若程序所在位置“不可写”（比如把程序放在只读U盘/只读目录/被系统限制的目录）：会自动改用桌面（或主目录），并在控制台用 `Work folder:` 提示实际路径
...
💡 性能提示：检测到 80 张待识别照片
   建议开启并行识别以加速处理
   预计可节省: 120秒 → 40秒（示例，实际取决于机器性能与照片情况）
```

**如何开启**（仅适用于源码版，打包版通常不需要）：
- 打包版默认使用更稳的设置；如需并行加速，请联系维护者协助评估与配置。
