...
│       └── group_photo_103045.jpg
└── unknown_photos/            # 未识别照片
    ├── Unknown_Person_1/      # 🆕 v1.3.0 相似人脸自动归组
    │   └── 2024-12-21/
    │       ├── visitor_a.jpg
    │       └── visitor_b.jpg
    └── 2024-12-21/            # 单次出现的人脸
...
- **Desktop/SundaySchoolPhotoOrganizer/** (首次运行自动创建):
  - `student_photos/`: 学生参考照（每人一个文件夹）
  - `class_photos/`: 课堂照片（建议按日期建子目录）
...
- **测试覆盖**: 128+ 测试用例，覆盖核心功能、性能、用户体验、打包部署

### 性能优化
- **智能并行识别**: 大批量照片（50+张）自动建议启用并行，性能提升 60-70%
- **多层缓存**: L1 内存缓存 + L2 磁盘缓存 + L3 增量快照，二次运行加速 70%+
