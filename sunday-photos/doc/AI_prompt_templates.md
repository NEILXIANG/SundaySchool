# AI 需求与提示词模板（适配本项目：老师默认无需参数的照片整理）
...
【最重要的产品原则】
- 老师端应尽量做到“默认无需参数、默认无需额外配置、操作尽量清晰”。
- 文档/启动脚本/可执行文件名/控制台提示语必须一致。

【老师端体验（必须实现）】
1) 老师双击运行打包程序。
2) 程序在可执行文件同目录自动创建 `input/`、`output/`、`logs/`（其中 `input/` 包含 `student_photos/`、`class_photos/`）：
   - student_photos/
   - class_photos/
   - output/
   - logs/
...
- K1 老师端应保持默认无需参数，否则容易增加求助成本。
- K2 入口/文档/脚本/产物名必须强一致，否则老师一定放错照片或运行错入口。
- K3 “自动按日期移动课堂照”属于正确行为但容易让人困惑，必须提前解释。
