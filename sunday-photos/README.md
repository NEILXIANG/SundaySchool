...
- 编辑桌面目录下 `主日学照片整理/config.json`：`face_recognition.tolerance`
...
### ✅ 简单易用
- ❌ 不需要CSV文件
- ✅ 不需要编辑配置文件（大多数情况）；也支持可选调参（tolerance）
- ❌ 不需要计算机知识
- ✅ 只需创建文件夹和放照片
...
### Q: 人脸识别不准确怎么办？
A: 1) 老师（打包版）：编辑桌面 `主日学照片整理/config.json`，调整 `face_recognition.tolerance`（建议 0.45-0.75）
   2) 开发者（源码）：可调整 `--tolerance` 参数（0.4-0.8之间）
   3) 确保参考照片清晰且包含完整正面人脸，并提供 2-3 张不同角度的参考照片
