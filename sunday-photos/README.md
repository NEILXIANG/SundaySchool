...
**本工具方案**：
- ✨ 一键运行，自动识别所有学生
- 🚀 常见规模的照片通常可在几分钟内完成（受机器性能与照片质量影响）
- 📊 自动生成详细统计报告
...
### 🧠 智能识别引擎
- **高精度识别**：默认使用 InsightFace（推荐）；也支持回退到 dlib/face_recognition（可配置）。识别效果会随照片清晰度、光线、角度与参考照质量而变化
...
### ⚡ 卓越性能架构
- **并行识别（更稳定的固定策略）**：
  - 小批量(<min_photos) → 默认串行（减少进程启动开销，调试更直观）
  - 大批量(≥min_photos) → 启用多进程并行
  - 异常自动回退 → 串行兜底，确保完成
- **增量处理引擎**：
  - 按日期文件夹追踪处理状态
  - 仅处理新增/变更的照片
...
程序会根据配置与照片数量自动选择串行/并行（默认开启并行；小批量更偏向串行更稳）：
...
# ⚡ 自动使用多核并行（示例；实际效果取决于机器与照片情况）
```

**场景3：自定义并行参数（可选）**
```bash
# 在 config.json 中设置：
# "parallel_recognition": { "enabled": true, "workers": 6, "min_photos": 30 }
...
│   └── 2024-12-21/                     # Date folder (normalized)
│       ├── group_photo.jpg
│       ├── discussion.jpg
│       └── showcase.jpg
├── Bob/
...
│   ├── 2024-12-21/
│   │   └── blurry.jpg
│   └── Unknown_Person_1/               # Clustered unknown faces
│       └── 2024-12-21/
│           └── visitor.jpg
...
本工具采用“稳定优先”的并行策略：小批量默认串行，大批量才启用并行；并行失败会自动回退串行，确保流程完成。

#### 🎯 工作原理

```
照片数量 < min_photos  → 串行（快速启动，稳定优先）
照片数量 ≥ min_photos  → 并行（多进程）
```

#### 📊 性能对比

> 说明：下表为示例数据，用于帮助理解量级；不同电脑与照片质量会显著影响耗时。
...
> 💡 **实践建议**：小批量串行通常更快更稳；大批量并行通常可明显加速（实际取决于 CPU/内存/照片数量与清晰度）。
...
// 在 config.json 中设置
{
  "parallel_recognition": {
    "enabled": true,
    "workers": 6,        // 默认=6（固定不智能拉高），更稳；上限不超过 CPU 核心数
...
1. **小批量回退**：照片数 < min_photos 自动串行，避免无效开销
...
# 首次看到提示后，临时启用
SUNDAY_PHOTOS_PARALLEL=1 python run.py
# ⚡ 多核加速，通常可明显节省时间（示例；实际取决于机器与照片情况）
```
...
### v0.4.0（当前版本）- 2025-12-26
- ✨ **重大新增**：未知人脸智能聚类（访客/家长自动归组）
- ✨ 人脸识别后端升级：默认 InsightFace；支持通过配置/环境变量切换 dlib/face_recognition
- ⚡ 并行策略更稳定：默认 workers 固定为 6，不做智能拉高；小批量自动串行
- 🧰 缓存隔离：参考照编码缓存按后端/模型分目录，避免 128/512 混用
- 🐛 修复 `pkg_resources` 警告噪音
- 📚 文档全面升级：体现工程能力和专业性

### v1.2.0 - 2025-12-20
- ⚡ 默认启用并行识别（固定默认 workers，行为更可预期）
