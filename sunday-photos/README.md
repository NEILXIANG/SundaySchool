...
### 🧠 智能识别引擎
- **高精度识别**：基于 dlib 深度学习模型；识别效果会随照片清晰度、光线、角度与参考照质量而变化
...
- **智能并行决策**：
  - 小批量(<30张) → 串行模式，快速启动
  - 大批量(≥50张) → 智能提示并行加速，通常可明显加速（效果取决于 CPU/内存/照片数量与清晰度）
  - 异常自动回退 → 串行兜底，确保完成
- **增量处理引擎**：
  - 按日期文件夹追踪处理状态
  - 仅处理新增/变更的照片
...
# ⚡ 自动使用多核并行，通常可明显加速（示例；实际取决于机器与照片情况）
```
...
### v0.4.0（当前版本）- 2025-12-23
- ✨ **重大新增**：未知人脸智能聚类（访客/家长自动归组）
- ✨ 新增智能并行决策系统
- ⚡ 性能优化：大批量场景通常可受益于并行加速（效果取决于机器与照片规模）
- 💡 智能提示：自动建议性能优化方案
